...
// 当滚动到底的时候做相关处理
function scorll(callback) {
    var curPage = 1;
    window.onscroll = function () {
        let sumH = document.body.scrollHeight || document.documentElement.scrollHeight;
        let viewH = document.documentElement.clientHeight;
        let scrollH = document.body.scrollTop || document.documentElement.scrollTop;
        // console.log('%s,%s,%s',viewH ,scrollH ,sumH);
        if (Math.floor(viewH + scrollH) + 1 >= sumH) {
            // 这里的“>=”完成页面高度的增加
            //  console.log('nzq')
            callback(++curPage)
        }
